    ...
    def on_epoch_end(self, epoch):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for annotation_line in tqdm(self.val_lines):
                ...
                image       = Image.open(line[0])  #读取图像并转换成RGB图像
                gt_boxes    = np.array([np.array(list(map(int,box.split(',')))) for box in line[1:]])  #获得预测框
                self.get_map_txt(image_id, image, self.class_names, self.map_out_path)  #获得预测txt
                with open(os.path.join(self.map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                    for box in gt_boxes:
                        left, top, right, bottom, obj = box
                        obj_name = self.class_names[obj]
                        new_f.write("%s %s %s %s %s\n" % (obj_name, left, top, right, bottom))  #获得真实框txt
